...
{   
    ...
    nowDate = nowDate == undefined ? new Date() : nowDate;
    ...
    {
        yyyy = nowDate.getUTCFullYear();
        mm = nowDate.getUTCMonth() + 1 + ''; mm = mm.length == 1 ? '0' + mm : mm;
    ...
    waitUntiFirstGraphLoaded(uriReq, graphDetails);
}
...
function parseSGPayload(payload)
{
    ...
    globalSingleDayGraphs.plain = {graph_json: gunzip.decompress()};
    console.log('\tDone uncompressing file');
}

function waitUntiFirstGraphLoaded(uriReq, graphDetails)
{
    //watch-0
    console.log('\nwaitUntiFirstGraphLoaded(), graphs:', globalSingleDayGraphs.graphs.length);

    if( globalSingleDayGraphs.graphs.length == 0 )
    {
        traverseJSONGraphs();
        setTimeout(function()
        {
    ...
    populateGraphSet( globalSingleDayGraphs.graphs );
...
function loadAdditionalGraphs()
{
    console.log('\nloadAdditionalGraphs()')
    //globalSingleDayGraphs.plain.cursor < globalSingleDayGraphs.plain.graph_json.length
...
function traverseJSONGraphs()
{
    console.log( '\ntraverseJSONGraphs(): ' );
    if( globalSingleDayGraphs.plain == undefined )
    {
        return;
    }

    if( globalSingleDayGraphs.plain.cursor == undefined )
    {
        globalSingleDayGraphs.plain.cursor = 0;
    }

    let graph = '';
    let lineChar = ''
    
    //JSON.parse happens on main thread
    for (; globalSingleDayGraphs.plain.cursor < globalSingleDayGraphs.plain.graph_json.length; globalSingleDayGraphs.plain.cursor++) 
    {         
        lineChar = String.fromCharCode( globalSingleDayGraphs.plain.graph_json[globalSingleDayGraphs.plain.cursor] );
        ...
        graph += lineChar;
        globalSingleDayGraphs.plain.graph_json[globalSingleDayGraphs.plain.cursor] = '';
    }
    
        
    /* traverse graphs backwards
        consider reversing allDayGraphs or using stack
        //method 1
        for (let i = plain.length-1; i > -1; i--) 
        {         
            ...
            if( lineChar == '\n' && graph.length > 5 )
            {
                ...
                {   
                    graph = JSON.parse(graph);
                    allDayGraphs.push(graph);
                    graph = '';
                } 
                catch(e) {console.log('Graph parse error:', e);}
            }
            graph = lineChar + graph;
            plain[i] = '';
        }
    */
...
function main(graphs, cursor)
{   
    let graph = graphs[cursor];
...
function graphDateLocalChange(cursor)
{
    cursor = isNaN(cursor) ? 0 : cursor; 
    let curDate = document.getElementById('graphDateLocal');
    if( curDate.value.length === 0 )
    {
        return;
    }

    let dateFrags = curDate.value.split('-');
    if( dateFrags.length != 3 && globalURIParamsDict.endpoint == undefined )
    {
        return;
    }

    let yyyy = dateFrags[0]; let mm = dateFrags[1]; let dd = dateFrags[2];
    let reqURI = getReqURIForEndPoint(globalURIParamsDict.endpoint, cursor, 0, yyyy, mm, dd);
    
    location.replace(reqURI);
...
function populateGraphSet(dayGraphs)
{
    //watch-5-done
    console.log('\npopulateGraphSet()');
...
function getReqURIForEndPoint(endpoint, cursor, hist, yyyy, mm, dd)
{
    if( endpoint == '/usa/' )
    {
        return '/storygraph/graphs/usa/#cursor=' + cursor + '&hist=' + hist + '&t=' + yyyy + '-' + mm + '-' + dd + 'T00:00:00';
    }
